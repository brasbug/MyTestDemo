...
- (void)accessColumnPointCollection {
    ...
    if (columns == 0) @throw [NSException exceptionWithName:@"" reason:@"number of content columns must more than 0" userInfo:nil];
...
//        cell.selectionStyle = UITableViewCellSelectionStyleNone;
    ...
    for (int i = 0; i < count; i++) {
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(2, 0, cellW - 4, cellH)];
...
- (void)contentHeaderTap:(UITapGestureRecognizer *)recognizer {
    ...
    if (section == -1) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            [columnSortedTapFlags setObject:[NSNumber numberWithInteger:columnFlag] forKey:str];
...
- (void)singleHeaderClick:(NSIndexPath *)indexPath {
    ...
    [columnSortedTapFlags setObject:[NSNumber numberWithInteger:columnFlag] forKey:columnStr];
