...
@interface JKPopMenu ()<UITableViewDelegate, UITableViewDataSource>
...
- (UIImageView *)menuContainerView {
    if (!_menuContainerView) {
        ...
        _menuContainerView.backgroundColor = [UIColor blackColor];
        _menuContainerView.userInteractionEnabled = YES;
        _menuContainerView.frame = CGRectMake(CGRectGetWidth(self.bounds) - JKMenuTableViewWidth - 6, 65, JKMenuTableViewWidth, self.menus.count * (JKMenuItemViewHeight + JKSeparatorLineImageViewHeight) + JKMenuTableViewSapcing);
...
- (UITableView *)menuTableView {
    if (!_menuTableView) {
        _menuTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, JKMenuTableViewSapcing, CGRectGetWidth(_menuContainerView.bounds), CGRectGetHeight(_menuContainerView.bounds) - JKMenuTableViewSapcing) style:UITableViewStylePlain];
        ...
        _menuTableView.rowHeight = JKMenuItemViewHeight;
...
- (instancetype)initWithObjects:(id)firstObj, ... NS_REQUIRES_NIL_TERMINATION {
    ...
    if (self) {
        ...
        JKPopMenuItem *eachItem;
        va_list argumentList;
        if (firstObj) {
            [menuItems addObject:firstObj];
            va_start(argumentList, firstObj);
            while((eachItem = va_arg(argumentList, JKPopMenuItem *))) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    JKPopMenuItemCell *popMenuItemView = (JKPopMenuItemCell *)[tableView dequeueReusableCellWithIdentifier:cellIdentifer];
    if (!popMenuItemView) {
        popMenuItemView = [[JKPopMenuItemCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifer];
    }
